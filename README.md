...
- **Εγκατάσταση σε κινητό:** Στην **Κεντρική οθόνη** (Dashboard) εμφανίζεται **QR code** με τη διεύθυνση της εφαρμογής. Σαρώστε το με το κινητό → ανοίγει η εφαρμογή στο browser → **«Πρόσθεσε στην αρχική οθόνη»** / **Add to Home Screen** (1–2 κλικ). Σε **production** (όταν η εφαρμογή είναι σε πραγματικό domain), το ίδιο QR μπορεί να μοιραστεί (π.χ. σε φίλο) ώστε να εγκαταστήσει την εφαρμογή χωρίς Play Store. **Δεν απαιτείται** ξαναγράψιμο σε Java/Kotlin· η εφαρμογή τρέχει ως PWA (React).
- **Τοπικό δίκτυο (development):** Αν τρέχετε `npm run dev` στον PC, το QR θα δείχνει `http://localhost:5173`· το κινητό **δεν** μπορεί να ανοίξει το localhost του PC. Χρειάζεται κινητό και PC στο **ίδιο WiFi**· ανοίξτε στο κινητό **χειροκίνητα** τη διεύθυνση `http://<IP-υπολογιστή>:5173` (το IP βρίσκεται στις ρυθμίσεις δικτύου του PC). Αν ανοίγει **κενή σελίδα**: (α) ελέγξτε ότι το κινητό και ο PC είναι στο ίδιο δίκτυο, (β) ότι ο dev server τρέχει με `npm run dev`, (γ) πιθανόν firewall να μπλοκάρει τη θύρα 5173· δοκιμάστε προσωρινά απενεργοποίηση ή κανόνα που να επιτρέπει το port.
- **Σημείωση ασφαλείας:** Οι διευθύνσεις `http://10.0.2.2:5173` (Android emulator) και `http://<IP-PC>:5173` (φυσικό κινητό) είναι **κανονικές διευθύνσεις τοπικού δικτύου**· δεν είναι ιός ούτε κακόβουλο λογισμικό. Χρησιμοποιούνται μόνο για πρόσβαση στον δικό σας dev server από emulator ή κινητό.
- **Android Studio emulator:** Τρέξιμο `npm run dev` στο host· στο emulator άνοιγμα `http://10.0.2.2:5173`· από εκεί μπορεί να γίνει και εγκατάσταση PWA.
